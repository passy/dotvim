...
srv = ServerProxy('https://paste.welcloud.de/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^http://paste.welcloud.de/show/([^/]+)/?$')
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    paste_id = new_paste(lng_code, code, parent, '', '', True)
    url = 'http://paste.welcloud.de/show/%s' % paste_id
